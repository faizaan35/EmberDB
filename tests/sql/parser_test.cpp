...
static emberdb::Result<std::unique_ptr<emberdb::Statement>> ParseSQL(const std::string& sql) {
...
TEST_CASE("Phase 3 Completion Gate: SQL Parser Statement Coverage", "[sql][parser][gate]") {
    SECTION("CREATE TABLE parsing") {
        ...
        REQUIRE(res.ok());
        auto* stmt = dynamic_cast<emberdb::CreateTableStatement*>(res->get());
        REQUIRE(stmt != nullptr);
        REQUIRE(stmt->GetTableName() == "users");
        REQUIRE(stmt->GetColumns().size() == 4);
        REQUIRE(stmt->GetColumns()[0].name == "id");
        REQUIRE(stmt->GetColumns()[0].type == emberdb::TypeId::INTEGER);
        REQUIRE(stmt->GetColumns()[1].name == "name");
        REQUIRE(stmt->GetColumns()[1].type == emberdb::TypeId::VARCHAR);
        ...
        REQUIRE(stmt->GetColumns()[3].type == emberdb::TypeId::DOUBLE);
    }
    ...
    SECTION("INSERT INTO parsing") {
        ...
        auto res = ParseSQL(sql);
        REQUIRE(res.ok());
        auto* stmt = dynamic_cast<emberdb::InsertStatement*>(res->get());
        REQUIRE(stmt != nullptr);
        REQUIRE(stmt->GetTableName() == "users");
        ...
        auto* stmt_multi = dynamic_cast<emberdb::InsertStatement*>(res_multi->get());
    ...
    SECTION("SELECT query parsing with projections, WHERE, ORDER BY, LIMIT") {
        ...
        auto res = ParseSQL(sql);
        REQUIRE(res.ok());
        auto* stmt = dynamic_cast<emberdb::SelectStatement*>(res->get());
        REQUIRE(stmt != nullptr);
    ...
    SECTION("SELECT * wildcard") {
        ...
        REQUIRE(stmt->GetSelectList()[0]->GetType() == emberdb::ExpressionType::STAR);
    }
    ...
    SECTION("JOIN query parsing (INNER & LEFT)") {
        ...
        REQUIRE(stmt->GetJoins()[0].type == emberdb::JoinType::INNER);
    ...
    SECTION("UPDATE parsing") {
        ...
        auto res = ParseSQL(sql);
        REQUIRE(res.ok());
        auto* stmt = dynamic_cast<emberdb::UpdateStatement*>(res->get());
        REQUIRE(stmt != nullptr);
        REQUIRE(stmt->GetTableName() == "users");
    ...
    SECTION("DELETE parsing") {
        ...
        auto res = ParseSQL(sql);
        REQUIRE(res.ok());
        auto* stmt = dynamic_cast<emberdb::DeleteStatement*>(res->get());
        REQUIRE(stmt != nullptr);
        REQUIRE(stmt->GetTableName() == "users");
        REQUIRE(stmt->GetWhereClause() != nullptr);
        ...
        REQUIRE(res_all.ok());
        auto* stmt_all = dynamic_cast<emberdb::DeleteStatement*>(res_all->get());
    ...
    SECTION("Transactions: BEGIN, COMMIT, ROLLBACK") {
        ...
        REQUIRE(res_b.ok());
        auto* b = dynamic_cast<emberdb::TransactionStatement*>(res_b->get());
        REQUIRE(b->GetTransactionType() == emberdb::TransactionType::BEGIN);

        auto res_c = ParseSQL("COMMIT;");
        REQUIRE(res_c.ok());
        auto* c = dynamic_cast<emberdb::TransactionStatement*>(res_c->get());
        REQUIRE(c->GetTransactionType() == emberdb::TransactionType::COMMIT);

        auto res_r = ParseSQL("ROLLBACK;");
        REQUIRE(res_r.ok());
        auto* r = dynamic_cast<emberdb::TransactionStatement*>(res_r->get());
        REQUIRE(r->GetTransactionType() == emberdb::TransactionType::ROLLBACK);
    }

    SECTION("CREATE INDEX and DROP TABLE") {
        auto res_idx = ParseSQL("CREATE INDEX idx_users_id ON users(id);");
        REQUIRE(res_idx.ok());
        auto* idx = dynamic_cast<emberdb::CreateIndexStatement*>(res_idx->get());
        ...
        REQUIRE(res_drop.ok());
        auto* drop = dynamic_cast<emberdb::DropTableStatement*>(res_drop->get());
...
TEST_CASE("SQL Parser syntax error handling", "[sql][parser][errors]") {
    SECTION("Empty SQL") {
        ...
        REQUIRE(res.status().code() == emberdb::StatusCode::InvalidSyntax);
    }
